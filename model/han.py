...
class HAN_Head(nn.Module):
    def __init__(self, is_high=False, conv=default_conv):
        super(HAN_Head, self).__init__()
        
        if is_high:
            n_resgroups = 6 # args.n_resgroups
            n_resblocks = 12 # args.n_resblocks
        else:
            n_resgroups = 4 # args.n_resgroups
            n_resblocks = 8 # args.n_resblocks

        n_feats = 64 # args.n_feats
        kernel_size = 3
        reduction = 32 # args.reduction 
        # scale = args.scale[0]
        act = nn.ReLU(True)
